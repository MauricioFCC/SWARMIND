...
def main() -> int:
    ...
    argument_parser.add_argument(
        ...
        help="Emitir resultado JSON parseable (status/errors) en lugar de texto plano",
    )
    ...
    for path in args.files:
        ...
        if errors:
            failed = True
            results.append({"file": str(path), "status": "FAIL", "errors": errors})
        else:
            results.append({"file": str(path), "status": "OK", "errors": []})
    if args.json:
        import json
        print(json.dumps(
            {"status": "FAIL" if failed else "OK", "files": results},
