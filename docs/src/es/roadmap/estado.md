...
- **ADR-0075 Skills/Agentes frontera (APLICADO)** — `skill_composition.py` (calls lazy + anti-ciclos + tiers invocation + poda set-compatibility, Pocock −63%) + `competence_model.py` (Beta posterior por agente×skill + Thompson anti-collapse + imp@k) + `fanout_gate.py` (anti-sobre-descomposición: baseline ≥80% → single, evita amplificación ×17.2). 20 tests, mutante del gate muerto. Diferidos: SkillRouter-embeddings (YAGNI a 34 skills), GDE sub-goals verificables (refactor planner, ADR futuro).
